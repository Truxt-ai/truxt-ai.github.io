...
export default function Dropdown({ children, title }: DropdownProps) {
    ...
    return (
        <ul>
            <li className='relative flex items-center gap-0.5 px-3 py-1' onMouseEnter={() => setDropdownOpen(true)} onMouseLeave={() => setDropdownOpen(false)}>
                <span className='flex cursor-pointer items-center text-gray-700 transition hover:text-gray-900'>{title}</span>
                <button className='shrink-0 p-1' aria-expanded={dropdownOpen} onClick={handleClick}>
